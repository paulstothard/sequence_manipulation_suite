...
function colorAlignProp(theDocument) {
  ...
  groupString = "GAVLI, FYW, CM, ST, KRH, DE, NQ, P"
    .replace(/\s/g, "")
    .toUpperCase();
  ...
  colorAlign(
    ...
    arrayOfGroups,
    theDocument.forms[0].elements[7].value,
    longestTitle
  );
  closePre();
  closeWindow();
      ...
      while (
        ...
        if (columnSeq.match(re).length / arrayOfSequences.length >= consensus) {
          output =
            output +
            '<span class="' +
            arrayOfSequences[jj].charAt(i).toString().toLowerCase() +
            '">' +
            arrayOfSequences[jj].charAt(i) +
            "</span>";
          i = i + 1;
